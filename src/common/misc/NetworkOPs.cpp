        ...
        do
        {
            for (auto const& uDirEntry : sleNode->getFieldV256 (sfIndexes))
            {
                ...
                case ltSKYWELL_STATE:
                    if (!jvObjects.isMember (jss::skywell_lines))
                    {
                        jvObjects[jss::skywell_lines] =
                                Json::Value (Json::arrayValue);
                    }

                    jvObjects[jss::skywell_lines].append (sleCur->getJson (0));
                    break;
...
{ // CAUTION: This is the old get book page logic
    ...
    auto uTransferRate = skywellTransferRate (lesActive, book.out.account);
